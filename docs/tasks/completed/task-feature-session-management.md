...
4. ⚠️ Реализовать FSM для управления сессиями (частично - состояния созданы, но не все используются)
5. ✅ Реализовать проверку активной сессии

## Задачи

- [x] Реализовать создание сессии в команде `/new_query`:
  - ✅ Создать сессию типа `query_with_kb`
  - ✅ Установить статус `active`
  - ✅ Сохранить session_id в FSM state
  - ✅ Уведомить пользователя
- [x] Реализовать создание сессии в команде `/new_chat`:
  - ✅ Создать сессию типа `empty_chat`
  - ✅ Установить статус `active`
  - ✅ Сохранить session_id в FSM state
  - ✅ Уведомить пользователя
- [x] Реализовать завершение сессии в команде `/end_query`:
  - ✅ Получить активную сессию пользователя
  - ✅ Обновить статус сессии на `completed`
  - ✅ Очистить FSM state
  - ✅ Уведомить пользователя
- [x] Реализовать отмену сессии в команде `/cancel`:
  - ✅ Получить активную сессию пользователя (через state.clear())
  - ⚠️ Обновить статус сессии на `cancelled` (не реализовано, только state.clear())
  - ✅ Очистить FSM state
  - ✅ Уведомить пользователя
- [⚠️] Реализовать FSM состояния для управления сессиями:
  - ✅ `WaitingForQuery` - ожидание запроса (создано в states.py)
  - ✅ `ProcessingQuery` - обработка запроса (создано в states.py)
  - ✅ `WaitingForConfirmation` - ожидание подтверждения отката (создано в states.py)
  - ⚠️ Состояния созданы, но не все используются в обработчиках
- [x] Реализовать проверку активной сессии перед обработкой сообщений
