...
# Интервал обновления статуса «Обрабатываю запрос…» / «Получаю ответ…» (секунды)
QUERY_PROGRESS_TIMER_INTERVAL=15

# ============================================
# Cursor CLI: таймауты (опционально)
# ============================================
# Секунды тишины до первого непустого вывода в stdout (не «общий лимит ответа»).
# Для тяжёлых запросов оставьте 600 или больше.
...
После `git pull` сверьте `.env` на сервере с актуальным `.env.example` в репозитории: новые опциональные переменные (например `QUERY_PROGRESS_TIMER_INTERVAL` для статуса долгого запроса) можно добавить без смены образа, но после правки `.env` перезапустите бота: `dc-prod up -d bot`.
